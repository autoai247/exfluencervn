      ...
      <div className="card bg-dark-600 border-2 border-dark-500 shadow-xl space-y-4">
        ...
        <div>
          <label className="text-xs text-gray-400 mb-2 block">
            {language === 'ko' ? '지역' : 'Khu vực'}
            {form.location.length > 0 && (
              <span className="ml-2 text-primary">{form.location.length}{language === 'ko' ? '개 선택' : ' đã chọn'}</span>
            )}
          </label>
          {[
            ...
            <div key={group.groupKo} className="mb-2">
              <div className="text-xs text-gray-600 mb-1">{language === 'ko' ? group.groupKo : group.groupVi}</div>
                ...
                {group.cities.map(city => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-all ${
                      form.location.includes(city.value)
          ...
          {form.location.length === 0 && (
            <p className="text-xs text-gray-600 mt-1">{language === 'ko' ? '선택 안 하면 전국 대상' : 'Không chọn = toàn quốc'}</p>
          )}
        </div>
